 ...
 *
 */
public class LaunchedBuffPrototype extends AbstractBuffPrototype
{
	...
	public SkillError onStart(EffectEntity effect, long currTime)
	{
		...
		for(GameObject target : targets)
		{
			...
			if((troopTarget.getFightStates().getMetaStates() & FightStateEnum.IMMUNE_LAUNCHED.getStates()) > 0)
			{
			...
			FightStateService.addStateEffect(troopTarget, FightStateEnum.LAUNCHED, effect);
	...
	public SkillError onExit(EffectEntity effect)
	{
		...
		for(GameObject target : targets)
		{
			...
			FightStateService.removeStateEffect(troop, FightStateEnum.LAUNCHED, effect);
