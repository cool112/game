...
/**
 * 反向治疗效果，对目标释放，对施法者治疗（因此施法者只能是单位）
 * 
 * @author seg
 *
 */
public class ReverseHealPrototype extends EffectPrototype
{
	/**
	 * 转化效率
	 */
	...
	public SkillError onActivate(EffectEntity effect, long currTime)
	{
		...
		for (GameObject target : targets)
		{
			...
			float targetHealMod = troopCaster.getAttrModList().getEffectModifiedTotal(ModifiedType.BE_HEAlED, effect);
			int heal = (int) (ap * transforEfficient + fixHeal);
			heal *= (1 + healModified + targetHealMod);
			heal += troopCaster.getMaxHp() * maxHpPercent;
			FightEngine.addHP(troopCaster, effect.getObjectId(), effect.getPrototypeId(), heal,
					HpUpdateType.ADD_HP);
		}
