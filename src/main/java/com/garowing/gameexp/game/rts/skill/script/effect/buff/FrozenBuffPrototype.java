 ...
 *
 */
public class FrozenBuffPrototype extends AbstractBuffPrototype
{
	...
	public SkillError onStart(EffectEntity effect, long currTime)
	{
		...
		for(GameObject target : targets)
		{
			...
			if((troopTarget.getFightStates().getMetaStates() & FightStateEnum.IMMUNE_FROZE.getStates()) > 0)
			{
			...
			FightStateService.addStateEffect(troopTarget, FightStateEnum.FROZE, effect);
	...
	public SkillError onExit(EffectEntity effect)
	{
		...
		for(GameObject target : targets)
		{
			...
			FightStateService.removeStateEffect(troop, FightStateEnum.FROZE, effect);
