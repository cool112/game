 ...
 *
 */
public class EffectTargetStatusModifiedBuffPrototype extends AbstractBuffPrototype
{
	/**
	 * 状态列表
	 */
	...
	public SkillError onStart(EffectEntity effect, long currTime)
	{
		...
		{
			FightStateEnum state = FightStateEnum.getEnum(status);
			if(state != null)
				statusMask |= state.getStates();
		}
		...
		for(GameObject target : targets)
		{
			...
			if(subEffectIds == null || subEffectIds.isEmpty())
				troopTarget.getAttrModList().addModifiedValue(type, effect, new ModifiedValueKey(ModifiedValueKey.DEFAULT_EFFECT_KEY, 0, statusMask), FixValue.valueOf(calType, percent));
			else
			{
				for(Integer effectId : subEffectIds)
				{
					troopTarget.getAttrModList().addModifiedValue(type, effect, new ModifiedValueKey(MathUtil.getHashCode(ModifiedValueKey.EFFECT_TYPE, effectId), 0, statusMask), FixValue.valueOf(calType, percent));
				}
			}
