...
/**
 * 冲锋效果，最小冲锋距离限制，碰撞产生临时效果
 * @author seg
 *
 */
public class ChargePrototype extends EffectPrototype
{
	/**
	 * 最小冲锋距离
	 */
	...
	private float[] chargeTo(Troop troopTarget, Troop troopCaster, float[] unitVector, EffectEntity effect, long currTime, float maxDistance)
	{
		...
		for (float dis = 1; dis <= maxDistance; ++dis)
		{
			...
			WarObjectInstance blockObj = WarMoveService.getInstance().getCollisonUnit(troopCaster, currentX, currentY, troopCaster.getCollisionRadius());
			if (blockObj != null)
			{
