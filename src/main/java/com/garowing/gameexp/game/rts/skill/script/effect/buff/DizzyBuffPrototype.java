 ...
 *
 */
public class DizzyBuffPrototype extends AbstractBuffPrototype
{
	...
	public SkillError onStart(EffectEntity effect, long currTime)
	{
		...
		for(GameObject target : targets)
		{
			...
			if((troopTarget.getFightStates().getMetaStates() & FightStateEnum.IMMUNE_DIZZY.getStates()) > 0)
			{
			...
			FightStateService.addStateEffect(troopTarget, FightStateEnum.DIZZY, effect);
	...
	public SkillError onExit(EffectEntity effect)
	{
		...
		for(GameObject target : targets)
		{
			...
			FightStateService.removeStateEffect(troop, FightStateEnum.DIZZY, effect);
