 ...
 *
 */
public class ShortCircuitBuffPrototype extends AbstractBuffPrototype
{
	...
	public SkillError onStart(EffectEntity effect, long currTime)
	{
		...
		for(GameObject target : targets)
		{
			...
			if((troopTarget.getFightStates().getMetaStates() & FightStateEnum.IMMUNE_SHORT_CIRCUIT.getStates()) > 0)
			{
			...
			FightStateService.addStateEffect(troopTarget, FightStateEnum.SHORT_CIRCUIT, effect);
	...
	public SkillError onExit(EffectEntity effect)
	{
		...
		for(GameObject target : targets)
		{
			...
			FightStateService.removeStateEffect(troop, FightStateEnum.SHORT_CIRCUIT, effect);
